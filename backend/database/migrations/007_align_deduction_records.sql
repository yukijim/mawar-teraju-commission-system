...
-- 2. Add warnings column to batches table to log IC conflicts
ALTER TABLE batches ADD COLUMN IF NOT EXISTS warnings TEXT;

-- 3. Check and rename legacy snapshot columns if they exist
DO $$
BEGIN
...
-- 4. Check and add new columns if they do not exist
...
-- 5. Set NOT NULL constraints on ic_number and name once columns are filled/verified
...
-- 6. Check and drop obsolete legacy columns if they exist
DO $$
BEGIN
